...
export default class RectDrawer extends Drawer {
  ...
  protected _drawPath(ctx: CanvasRenderingContext2D, layer: Layer) {
    ...
    if (this.rounds?.lt) {
      ...
      ctx.quadraticCurveTo(-width / 2, -height / 2, -width / 2 + this.rounds.lt, -height / 2);
    } else {
    ...
    if (this.rounds?.rt) {
      ...
      ctx.quadraticCurveTo(width / 2, -height / 2, width / 2, -height / 2 + this.rounds.rt);
    } else {
    ...
    if (this.rounds?.rb) {
      ...
      ctx.quadraticCurveTo(width / 2, height / 2, width / 2 - this.rounds.rb, height / 2);
    } else {
      ctx.lineTo(width / 2, height / 2);
    }
    ...
    if (this.rounds?.lb) {
      ...
      ctx.quadraticCurveTo(-width / 2, height / 2, -width / 2, height / 2 - this.rounds.lb);
    } else {
      ctx.lineTo(-width / 2, height / 2);
    }
    ctx.closePath();
